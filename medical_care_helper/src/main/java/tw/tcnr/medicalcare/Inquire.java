...
public class Inquire extends AppCompatActivity {
    ...
    String msg = null;
    private String[] MYCOLUMN = new String[]{"id", "name", "record", "required", "types", "details", "startdate", "starttime", "enddate", "endtime"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < recSet_record_list.size(); i++) {
            ...
            item.put("end", fld[9]);
            item.put("hour", q3);
            item.put("note", fld[1]);  // 先顯示姓名吧  原本是5 細項
...
//        sqlctl = "SELECT * FROM member record";
        ...
        try {
            String result = DBConnector.executeQuery_Inquire(nameValuePairs);
//=========================================
            chk_httpstate();
            //=========================================
            ...
            if (jsonArray.length() > 0) { // MySQL 連結成功有資料
                //--------------------------------------------------------
                int rowsAffected = dbHper.clearRec_Inquire();                 // 匯入前,刪除所有SQLite資料
                //--------------------------------------------------------
                // 處理JASON 傳回來的每筆資料
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    // -------------------加入SQLite---------------------------------------
                    long rowID = dbHper.insertRec_m_Inqire(newRow);
                }
                // ---------------------------
            } else {
