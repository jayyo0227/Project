...
public class Record_main extends AppCompatActivity implements View.OnClickListener, View.OnTouchListener {
    ...
    private String[] MYCOLUMN = new String[]{"id", "name", "record", "required", "types", "details", "startdate", "starttime", "enddate", "endtime"};  // , "type", "detail", "start_time", "end_time"
    String msg = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bottomNav2.setSelectedItemId(R.id.nav_book);
        bottomNav3.setVisibility(View.VISIBLE);
        bottomNav4.setVisibility(View.GONE);

        DBConnector.connect_ip = "https://medicalcarehelper.com/tcnr25/android_connect_db_all.php";
        initDB();
        dbmysql();
        recSet = dbHper.getRecSet_record(); //all data  資料全部讀近來
        ...
        switch (v.getId()) {
            case R.id.btnAdd:
                ...
                if (tname.equals("") || trecord.equals("") || trequire.equals("")) {
                    Toast.makeText(Record_main.this, "資料空白無法新增 !", Toast.LENGTH_SHORT).show();
                } else {
                    //===========================================寫入MYSQL================================================
...
//                dialogtext.setText(getString(R.string.h01_b001));//設定與按鈕相同文字
    ...
    private void modify_mysql_update() {
        ...
        Toast.makeText(getApplicationContext(), "資料已修改完成,請重新載入!", Toast.LENGTH_SHORT).show();
...
//            Log.d(TAG, "httpstate=" + DBConnector.httpstate);
            ...
            if (jsonArray.length() > 0) { // MySQL 連結成功有資料
                //--------------------------------------------------------
                int rowsAffected = dbHper.clearRec_record(); // 匯入前,刪除所有SQLite資料
                //--------------------------------------------------------
                // 處理JASON 傳回來的每筆資料
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    // -------------------加入SQLite---------------------------------------
                    long rowID = dbHper.insertRec_record(newRow);//寫入SQLite
